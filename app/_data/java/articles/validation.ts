...
export const articles: JavaArticleDetail[] = [
{
    ...
    lead: "消費税計算は金額を扱うシステムで必ず必要になる処理ですが、端数処理の方式（四捨五入、切り捨て、切り上げ）によって1円の差が生まれます。double や float では誤差が避けられないため、業務用途では BigDecimal を使うのが鉄則です。この記事では、税抜から税込への変換、税込から税抜への逆算、軽減税率（8%）と標準税率（10%）の使い分けを BigDecimal で実装します。RoundingMode の選び方や、明細と合計で端数処理のタイミングが異なる場合の対応も含めて整理します。",
    useCases: [
  ...
  },
{
    ...
    lead: "売上の前年比、KPI の達成率、原価率の算出など、割合計算は業務システムのあらゆる場面で登場します。double で計算すると丸め誤差が蓄積されるため、帳票や報告書の数値が微妙にずれるリスクがあります。とくに明細行ごとの割合を積み上げて合計を出す処理では、蓄積誤差が最終行で顕在化し、帳票の合計欄が手計算と一致しないという指摘につながることがあります。この記事では、BigDecimal を使った割合の算出、増減率の計算、百分率から実数への逆算を整理します。MathContext でスケールと丸めモードを管理する方法も紹介します。",
    useCases: [
  ...
  },
{
    ...
    lead: "長さ、重量、温度、速度などの単位変換は、物流、製造、科学技術計算などの業務で頻繁に必要になります。変換式を個別に書くと組み合わせ爆発が起きるため、基準単位を経由する設計が有効です。温度変換のように単純な係数乗算では対応できない単位系もあり、華氏と摂氏の変換ミスは海外取引先との仕様書やり取りで実際に起こりやすい問題です。この記事では、enum で単位を定義し、基準単位への変換係数を持たせることで、任意の単位間の変換を共通ロジックで扱う方法を紹介します。",
    useCases: [
