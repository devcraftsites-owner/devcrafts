  ...
  },
{
    ...
    lead: "日本の業務システムでは、半角カナと全角カナの混在が日常的に発生します。銀行の全銀フォーマットは半角カナを要求し、画面入力やデータベースには全角カナが格納されていることが多いため、相互変換の処理は避けて通れません。単純な1文字ずつの置換で済めばよいのですが、半角カナの濁音（ｶﾞ）は基本文字 + 濁点の2文字で表現されるのに対し、全角カナの濁音（ガ）は1文字です。この2文字結合の処理を正しく実装しないと、変換結果がおかしくなります。\n\n見落としがちなのが文字化けです。Java の内部エンコーディングは UTF-16 ですが、業務システムでは Shift_JIS（または Windows-31J / MS932）のファイルを扱う場面が多く、読み込み時のエンコーディング指定を誤ると半角カナが意図しない文字に化けて、変換処理自体が正しく動きません。カナ変換とエンコーディング処理はセットで確認しておくと安全です。\n\n変換自体は Pure Java だけで完結しますが、ひらがな・ローマ字変換も含む複合的な変換が必要な場合は ICU4J の Transliterator が有力な選択肢です。この記事では、変換テーブルの設計、濁音・半濁音の結合ロジック、文字化けを防ぐエンコーディング指定まで、実務で必要な実装を整理します。",
    useCases: [
    ...
    cautions: [
      ...
      "ファイル I/O でのエンコーディング不一致が文字化けの主因になる。InputStreamReader / OutputStreamWriter には必ず Charset.forName(\"MS932\") または Charset.forName(\"Shift_JIS\") を明示し、デフォルトエンコーディング（JVM 起動ロケール依存）に頼らないこと。MS932（Windows-31J）は Shift_JIS の Microsoft 拡張版で、①や㈱などの拡張文字を含む業務ファイルでは MS932 の方が安全",
      "Shift_JIS ファイルを誤って ISO-8859-1 や UTF-8 で読み込むと、半角カナのバイト値（0xA1〜0xDF）が別の文字として解釈される不可逆な文字化けになる。一度化けた文字は変換テーブルでは元に戻せないため、ファイル受け取り時のエンコーディング確認をチェックリストに含めること",
    ],
...
switch (classify(c, next)) {
    ...
    libraryComparison: [
      { name: "標準 API（String + Map）", whenToUse: "半角カナ・全角カナの変換だけで済む場合。外部依存なしで変換テーブルを管理したいとき。", tradeoff: "変換テーブルの定義が冗長になる。ただし業務要件に合わせた細かい制御がしやすく、Shift_JIS / MS932 との組み合わせも明示的に制御できる。" },
      { name: "ICU4J（Transliterator）", whenToUse: "半角・全角変換だけでなく、ひらがな・カタカナ変換やローマ字変換も必要な場合。Unicode 正規化（NFC/NFD）と組み合わせた多言語テキスト処理。", tradeoff: "JAR サイズ約12MB と大きく、カナ変換だけのために導入するのは過剰。使う場合は Transliterator t = Transliterator.getInstance(\"[\\\\uFF61-\\\\uFF9F] Halfwidth-Fullwidth\"); のようにフィルタ付きで取得し、t.transliterate(str) を呼ぶ。フィルタなしでは半角英数字まで全角化されるため注意。" },
      { name: "Apache Commons Lang（StringUtils）", whenToUse: "基本的な文字列操作ユーティリティと合わせて使いたい場合。", tradeoff: "半角カナ・全角カナの変換機能は提供されていないため、変換ロジック自体は自前で実装する必要がある。依存を追加する意味が薄い。" },
    ],
    faq: [
      { question: "ICU4J の Transliterator で半角カナだけを全角カナに変換するにはどうしますか。", answer: "Transliterator.getInstance(\"[\\uFF61-\\uFF9F] Halfwidth-Fullwidth\") でインスタンスを生成し transliterate(str) を呼びます。U+FF61〜FF9F の範囲フィルタを付けないと半角英数字まで全角化されるため、フィルタは必須です。逆変換は \"Fullwidth-Halfwidth\" を使います。" },
      { question: "Shift_JIS ファイルに含まれる半角カナを Java で正しく読み込むにはどうすればよいですか。", answer: "new InputStreamReader(stream, Charset.forName(\"MS932\")) のようにエンコーディングを明示します。MS932（Windows-31J）は Shift_JIS の拡張版で業務ファイルでは安全です。誤って ISO-8859-1 で読み込むと半角カナのバイト値が別文字として解釈され、変換テーブルでは元に戻せない文字化けになります。" },
      { question: "往復変換（半角→全角→半角）で元に戻らないケースはありますか。", answer: "「ヴ」（ｳﾞ）を変換テーブルに含めた場合、全角「ヴ」を半角に戻す処理がなければ往復で一致しません。業務で使う文字範囲を決めてテーブルが対称になっているかテストで確認するのが確実です。" },
    ],
