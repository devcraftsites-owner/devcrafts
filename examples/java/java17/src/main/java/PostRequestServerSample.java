...
public class PostRequestServerSample {
    ...
    // Java 17: record でフォームデータを不変クラスとして表現
    record FormData(String name, String age) {
        // 空文字チェックを record 内にまとめる
        boolean isValid() {
    ...
    static void handleRequest(Socket socket) throws IOException {
        try (var in = socket.getInputStream();
            ...
            // ルーティング（Java 17: テキストブロックで HTML を読みやすく）
            if ("GET".equals(method) && "/".equals(path)) {
                ...
                System.out.println("フォームデータ: name=" + formData.name() + ", age=" + formData.age());
